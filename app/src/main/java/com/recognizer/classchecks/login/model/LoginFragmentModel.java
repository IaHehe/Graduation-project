...
public class LoginFragmentModel {
    ...
    public void login(final String phone, final String smsCode, boolean isVerifySmsCode, String regId, final Callback callback) {
        ...
        reqParams.put("isVerifySmsCode", String.valueOf(false)); //String.valueOf(isVerifySmsCode)
        reqParams.put("regId", regId);
        reqParams.put("ts", String.valueOf(System.currentTimeMillis()));
        HttpUtils.doPost(HttpAddrConstants.USER_LOGIN, reqParams, new okhttp3.Callback() {
            @Override
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if(response.isSuccessful()) {
                    if(response.code() == 200) {
                        Type jsonType = new TypeToken<BasicEntityData<StudentBean>>(){}.getType();
                        BasicEntityData<StudentBean> beData = GsonUtil.GsonToBean(responseBody, BasicEntityData.class, jsonType);
