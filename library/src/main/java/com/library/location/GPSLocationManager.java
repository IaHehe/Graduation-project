...
public class GPSLocationManager {
    ...
    public void start(final GPSLocationListener gpsLocationListener, boolean isOpenGps) {
        ...
        // 获取位置提供器，GPS、WIFI、NetWork
        List<String> providers = locationManager.getProviders(true);
        Location location = null;
        if(providers.contains(LocationManager.GPS_PROVIDER)) {
            LogUtil.i("GPS定位");
            mLocateType = LocationManager.GPS_PROVIDER;
            location = locationManager.getLastKnownLocation(mLocateType);
        }
        if(null == location && providers.contains(LocationManager.PASSIVE_PROVIDER)) {
            LogUtil.i("WIFI定位");
            mLocateType = LocationManager.PASSIVE_PROVIDER;
            location = locationManager.getLastKnownLocation(mLocateType);
        }
        ...
        if(location != null) {
            mGPSLocation.onLocationChanged(location);
        }
