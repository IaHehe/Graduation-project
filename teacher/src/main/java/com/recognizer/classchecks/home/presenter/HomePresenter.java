        ...
        mHomeModel.checkHasCourse(jwAccount, new HomeModel.Callback() {
            ...
            public void onSuccess(Object obj) {
                ...
                if("0".equals(ccb.getApi_code())) {
                    if(ccb.getApi_body().getHasCourse() == true) { // 有课，可以考勤

                        if(ccb.getApi_body().getNeedAttendance() == true) {
                            getView().showResult(HomeView.CAN_OK_CLOCKIN, ccb.getApi_message());
                        } else {
                            getView().showResult(HomeView.SHOW_MSG, "当前课程已考勤，不可重复操作");
                        }
                    } else { // 服务器返回当前没课
                ...
                } else if("-2".equals(ccb.getApi_code())) {
                    getView().showResult(HomeView.SHOW_MSG, HttpRequestCode.REQUEST_CONNECT_ERROR[1]);
                }
            }
