...
public class RegisterFragmentPresenter extends BasePresenter<RegisterFragmentView> {
    ...
    public void register() {
        ...
        mRegisterModel.register(phone, mRegisterView.getSMSCode(), regID, new RegisterFragmentModel.Callback() {
            @Override
            ...
            public void onFailed(Object obj) {
                mRegisterView.hideLoading();
                BasicEntity be = (BasicEntity) obj;
                if(HttpRequestCode.REQUEST_CONNECT_OVERTIME[0].equals(be.getCode())) {
                    mRegisterView.showMsg(be.getMessage());
                } else if(HttpRequestCode.REQUEST_NETWORK_ERROR[0].equals(be.getCode())) {
                    mRegisterView.showMsg(be.getMessage());
                } else if(HttpRequestCode.REQUEST_BUSYNESS[0].equals(be.getCode())) {
                    mRegisterView.showMsg(be.getMessage());
                }
            }
        });
    ...
    public void reqSMSCode() {
        ...
        mRegisterModel.reqSMSCode(phone, new RegisterFragmentModel.Callback() {
            ...
            public void onFailed(Object obj) {
                BasicEntity be = (BasicEntity) obj;
                if(HttpRequestCode.REQUEST_NETWORK_ERROR[0].equals(be.getCode())) {
                    mRegisterView.reqSMSStatus(false, be.getMessage());
                }
            }
        });
